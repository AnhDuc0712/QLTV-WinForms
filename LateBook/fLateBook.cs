    ...
    public partial class fLateBook : Form
    {
        ...
        private void LoadLateBooks(string keyword = "")
        {
            using (var db = new LibraryContext())
            {
                ...
                lblTotalLateBooks.Text = "T?ng s? sách tr? h?n: " + result.Count;
                lblTotalUser.Text = "T?ng ??c Gi?: " + db.Users.Count();

                if (dataGridView1.Columns.Count > 0)
                {
                    dataGridView1.Columns["HoTen"].HeaderText = "H? Tên";
                    dataGridView1.Columns["TenSach"].HeaderText = "Tên Sách";
                    dataGridView1.Columns["NgayMuon"].HeaderText = "Ngày M??n";
                    dataGridView1.Columns["HanTra"].HeaderText = "H?n Tr?";
                    dataGridView1.Columns["NgayTraThucTe"].HeaderText = "Ngày Tr? Th?c T?";
                    dataGridView1.Columns["SoNgayTre"].HeaderText = "S? Ngày Tr?";
